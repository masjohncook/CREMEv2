...
class DataLoggerServer(Machine, implements(IConfiguration), implements(IConfigurationCommon),
    ...
    def configure_base(self):
        filename_path = "00configuration/./DataLoggerServer_base.sh"
        parameters = [self.ip, self.username, self.password]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_data_collection(self):
        filename_path = "00configuration/./DataLoggerServer_data_collection.sh"
    ...
    def start_collect_data(self):
        filename_path = "01data_collection/./start_packet.sh"
    ...
    def download_atop_data(self, data_logger_client):
        filename_path = "01data_collection/./download_atop_data.sh"
    ...
    def download_log_data(self, data_logger_client, remote_path, remote_log, new_log):
        filename_path = "01data_collection/./download_log_data.sh"
        parameters = [self.ip, self.username, self.password, data_logger_client.ip, data_logger_client.username,
...
class DataLoggerClient(Machine, implements(IConfigurationCommon), implements(IDataCollection)):
    ...
    def configure_base(self):
        filename_path = "00configuration/./DataLoggerClient_base.sh"
        parameters = [self.ip, self.username, self.password]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)
    ...
    def configure_data_collection(self):
        ...
        filename_path = "00configuration/./DataLoggerClient_data_collection.sh"
        parameters = [self.ip, self.username, self.password, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path, self.dls.ip, rsyslog_file]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def start_collect_data(self):
        filename_path = "01data_collection/./start_atop.sh"
    ...
    def stop_collect_data(self):
        filename_path = "01data_collection/./stop_atop.sh"
...
class VulnerableClient(DataLoggerClient, implements(IConfiguration), implements(IConfigurationCommon),
    ...
    def configure_benign_services(self):
        filename_path = "00configuration/./Client_benign_services.sh"
    ...
    def configure_mirai(self):
        filename_path = "00configuration/./VulnerableClient_mirai.sh"
        parameters = [self.ip, self.username, self.password, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)
    ...
    def start_benign_behaviors(self):
        filename_path = "00configuration/./Client_start_benign_behaviors.sh"
        parameters = [self.hostname, self.ip, self.username, self.password, self.path, self.ftp_folder,
    ...
    def tmp_noexec(self):
        filename_path = "00configuration/./VulnerableClient_tmp_noexec.sh"
...
class TargetServer(DataLoggerClient, implements(IConfiguration), implements(IConfigurationCommon),
    ...
    def configure_data_collection(self):
        if self.rsyslog_apache:
            ...
            rsyslog_file = "rsyslog_no_apache.conf"
        filename_path = "00configuration/./TargetServer_data_collection.sh"
        parameters = [self.ip, self.username, self.password, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path, self.dls.ip, rsyslog_file]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_benign_services(self):
        filename_path = "00configuration/./Server_benign_services.sh"
        ...
        for client in self.vulnerable_clients:
            filename_path = "00configuration/./Server_create_FTP_user.sh"
    ...
    def configure_resource_hijacking(self):
        filename_path = "00configuration/./TargetServer_resource_hijacking.sh"
        parameters = [self.ip, self.username, self.password]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)
    ...
    #about persistent 00configuration after reboot
    def configure_end_point_dos_ulimit(self):
        filename_path = "00configuration/./TargetServer_end_point_dos_ulimit.sh"
        parameters = [self.ip, self.username, self.password]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)
...
class BenignServer(DataLoggerClient, implements(IConfiguration), implements(IConfigurationCommon),
    ...
    def configure_data_collection(self):
        if self.rsyslog_apache:
            ...
            rsyslog_file = "rsyslog_no_apache.conf"
        filename_path = "00configuration/./TargetServer_data_collection.sh"  # similar to target_server
        parameters = [self.ip, self.username, self.password, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path, self.dls.ip, rsyslog_file]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)
...
class AttackerServer(Machine, implements(IConfiguration), implements(IConfigurationCommon),
    ...
    def configure_base(self):
        filename_path = "00configuration/./AttackerServer_base.sh"
        parameters = [self.ip, self.username, self.password, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path, self.DNS_server_ip]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_data_collection(self):
        filename_path = "00configuration/./AttackerServer_data_collection.sh"
    ...
    def configure_mirai(self):
        ...
        filename_path = "00configuration/./AttackerServer_mirai.sh"
    ...
    def configure_pymetasploit(self):
        filename_path = "00configuration/./AttackerServer_pymetasploit.sh"
        parameters = [self.ip, self.username, self.password, self.path]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_apache2(self):
        filename_path = "00configuration/./AttackerServer_apache2.sh"
        parameters = [self.ip, self.username, self.password]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_ransomware(self):
        prepared_files = "CREME/CREME_backend_execution/scripts/00configuration/prepared_files/ransomware/attacker_server"
        filename_path = "00configuration/./AttackerServer_ransomware.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path, prepared_files]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_resource_hijacking(self):
        prepared_files = "CREME/CREME_backend_execution/scripts/00configuration/prepared_files/resource_hijacking/attacker_server"
        filename_path = "00configuration/./AttackerServer_resource_hijacking.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path, prepared_files]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_disk_wipe(self):
        prepared_files = "CREME/CREME_backend_execution/scripts/00configuration/prepared_files/disk_wipe/attacker_server"
        filename_path = "00configuration/./AttackerServer_disk_wipe.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path, prepared_files]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_end_point_dos(self):
        prepared_files = "CREME/CREME_backend_execution/scripts/00configuration/prepared_files/end_point_dos/attacker_server"
        filename_path = "00configuration/./AttackerServer_end_point_dos.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path, prepared_files]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_data_theft(self):
        prepared_files = "CREME/CREME_backend_execution/scripts/00configuration/prepared_files/data_theft/attacker_server"
        filename_path = "00configuration/./AttackerServer_data_theft.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path, prepared_files]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_rootkit_ransomware(self):
        prepared_files = "CREME/CREME_backend_execution/scripts/00configuration/prepared_files/rootkit_ransomware/attacker_server"
        filename_path = "00configuration/./AttackerServer_rootkit_ransomware.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path, prepared_files]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def mirai_start_cnc_and_login(self):
        filename_path = "02Scenario/mirai/./AttackerServer_start_cnc_and_login.sh"
    ...
    def mirai_wait_for_finished_scan(self):
        ...
        filename_path = "02Scenario/mirai/./AttackerServer_wait_for_finished_phase.sh"
    ...
    def mirai_transfer_and_start_malicious(self):
        ...
        filename_path = "02Scenario/mirai/./AttackerServer_transfer_and_start_malicious.sh"
    ...
    def stop_malicious(self):
        filename_path = "02Scenario/mirai/./AttackerServer_cnc_stop_malicious.sh"
    ...
    def disk_wipe_start_metasploit(self):
        filename_path = "02Scenario/disk_wipe/./AttackerServer_start_metasploit.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.killed_pids_file]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def disk_wipe_first_stage(self):
        filename_path = "02Scenario/disk_wipe/./AttackerServer_first_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def disk_wipe_second_stage(self):
        filename_path = "02Scenario/disk_wipe/./AttackerServer_second_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def disk_wipe_third_stage(self):
        filename_path = "02Scenario/disk_wipe/./AttackerServer_third_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def ransomware_start_metasploit(self):
        filename_path = "02Scenario/ransomware/./AttackerServer_start_metasploit.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.killed_pids_file]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def ransomware_first_stage(self):
        filename_path = "02Scenario/ransomware/./AttackerServer_first_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def ransomware_second_stage(self):
        filename_path = "02Scenario/ransomware/./AttackerServer_second_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def ransomware_third_stage(self):
        filename_path = "02Scenario/ransomware/./AttackerServer_third_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def resource_hijacking_start_metasploit(self):
        filename_path = "02Scenario/resource_hijacking/./AttackerServer_start_metasploit.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.killed_pids_file]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def resource_hijacking_first_stage(self):
        filename_path = "02Scenario/resource_hijacking/./AttackerServer_first_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def resource_hijacking_second_stage(self):
        filename_path = "02Scenario/resource_hijacking/./AttackerServer_second_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def resource_hijacking_third_stage(self):
        filename_path = "02Scenario/resource_hijacking/./AttackerServer_third_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def end_point_dos_start_metasploit(self):
        filename_path = "02Scenario/end_point_dos/./AttackerServer_start_metasploit.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.killed_pids_file]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def end_point_dos_first_stage(self):
        filename_path = "02Scenario/end_point_dos/./AttackerServer_first_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)
    ...
    def end_point_dos_second_stage(self):
        ...
        filename_path = "02Scenario/end_point_dos/./AttackerServer_second_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack,
    ...
    def end_point_dos_third_stage(self):
        ...
        new_user_password = "password"
        filename_path = "02Scenario/end_point_dos/./AttackerServer_third_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack,
                      new_user_account, new_user_password]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def data_theft_start_metasploit(self):
        filename_path = "02Scenario/data_theft/./AttackerServer_start_metasploit.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.killed_pids_file]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def data_theft_first_stage(self):
        filename_path = "02Scenario/data_theft/./AttackerServer_first_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def data_theft_second_stage(self):
        filename_path = "02Scenario/data_theft/./AttackerServer_second_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def data_theft_third_stage(self):
        filename_path = "02Scenario/data_theft/./AttackerServer_third_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def rootkit_ransomware_start_metasploit(self):
        filename_path = "02Scenario/rootkit_ransomware/./AttackerServer_start_metasploit.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.killed_pids_file]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def rootkit_ransomware_first_stage(self):
        filename_path = "02Scenario/rootkit_ransomware/./AttackerServer_first_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def rootkit_ransomware_second_stage(self):
        filename_path = "02Scenario/rootkit_ransomware/./AttackerServer_second_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def rootkit_ransomware_third_stage(self):
        filename_path = "02Scenario/rootkit_ransomware/./AttackerServer_third_stage.sh"
        parameters = [self.ip, self.username, self.password, self.path, self.targeted_attack]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)
...
class MaliciousClient(Machine, implements(IConfiguration), implements(IConfigurationCommon),
    ...
    def configure_base(self):
        filename_path = "00configuration/./MaliciousClient_base.sh"
        parameters = [self.ip, self.username, self.password, self.controller_ip, self.controller_username,
                      self.controller_password, self.controller_path, self.DNS_server_ip]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_data_collection(self):
        filename_path = "00configuration/./MaliciousClient_data_collection.sh"
        parameters = [self.ip, self.username, self.password, self.data_logger_server_ip]
        ScriptHelper.execute_script(filename_path, parameters, self.show_cmd)

    def configure_mirai(self):
        filename_path = "00configuration/./MaliciousClient_mirai.sh"
    ...
    def mirai_start_malicious(self):
        ...
        filename_path = "02Scenario/mirai/./MaliciousClient_start_malicious.sh"
    ...
    def mirai_stop_malicious(self):
        filename_path = "02Scenario/mirai/./MaliciousClient_stop_malicious.sh"
